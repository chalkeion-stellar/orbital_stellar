...
Multiple hook instances with the same `(serverUrl, address, eventKey)` arguments share a single `EventSource` connection. The connection is closed when the last instance unmounts.
...
| **No loading skeleton** | You cannot render a partial UI (e.g. a skeleton with a spinner) inside the suspended component. Put that UI in the `fallback` prop of `<Suspense>` instead. |
